...
export class MeshStack extends Stack {
  ...
  constructor(sd: ServiceDiscoveryStack, id: string, props?: StackProps) {
    ...
    this.virtualNodeListender = appmesh.VirtualNodeListener.http({
      port: sd.base.containerPort,
    });

    this.backendV1VirtualNode = new appmesh.VirtualNode(this, `${this.prefix}BackendV1VirtualNode`, {
      mesh: sd.base.mesh,
      virtualNodeName: `${sd.base.projectName}-backend-v1-node`,
      listeners: [this.virtualNodeListender],
      serviceDiscovery: appmesh.ServiceDiscovery.dns(sd.backendV1LoadBalancer.loadBalancerDnsName)
    });

    this.backendV2VirtualNode = new appmesh.VirtualNode(this, `${this.prefix}BackendV2VirtualNode`, {
      mesh: sd.base.mesh,
      virtualNodeName: `${sd.base.projectName}-backend-v2-node`,
      listeners: [this.virtualNodeListender],
    ...
    this.backendVirtualRouter = new appmesh.VirtualRouter(this, `${this.prefix}BackendVirtualRouter`, {
      mesh: sd.base.mesh,
      virtualRouterName: `${sd.base.projectName}-backend-router`,
      listeners: [this.virtualNodeListender]
    });

    this.backendVirtualService = new appmesh.VirtualService(this, `${this.prefix}BackendVirtualService`, {
      virtualServiceProvider: appmesh.VirtualServiceProvider.virtualRouter(this.backendVirtualRouter),
      virtualServiceName: `backend.${sd.base.dnsHostedZone.zoneName}`,
    });
    ...
    this.backendRoute = new appmesh.Route(this, `${this.prefix}BackendRoute`, {
      ...
      routeName: `${sd.base.projectName}-backend-route`,
      routeSpec: routeSpec
    });
    ...
    this.frontendVirtualNode = new appmesh.VirtualNode(this, `${this.prefix}FrontendVirtualNode`, {
      mesh: sd.base.mesh,
      virtualNodeName: `${sd.base.projectName}-front-node`,
      listeners: [this.virtualNodeListender],
      serviceDiscovery: appmesh.ServiceDiscovery.dns(sd.frontendLoadBalancer.loadBalancerDnsName),
    });
