...
WORKDIR /go/src/github.com/aws/aws-app-mesh-examples/walkthroughs/howto-multiple-listeners/src/teller
...
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix nocgo -o /src/teller .
...
RUN yum update -y && \
    ...
    yum clean all && \
    rm -rf /var/cache/yum
COPY --from=builder /src/teller /src/teller

ENTRYPOINT ["/src/teller"]
