...
main() {
    ...
    update)
        case "$update_resource" in
        vnode)
            update_vnode "${TEST_MESH_DIR}/${resource_name}-vn-update.json" "${resource_name}-vn" "${resource_name}"
            ;;
            
        vgateway)
            update_vgateway "${TEST_MESH_DIR}/${resource_name}-vg-update.json" "${resource_name}-vg"
            ;;

        *)
            err "Invalid resource provided for update: $update_resource"
            ;;
        esac
        ;;
