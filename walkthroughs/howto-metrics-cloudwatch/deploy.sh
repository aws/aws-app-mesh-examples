...
STATSD_CW_IMAGE="${ECR_IMAGE_PREFIX}/statsd-to-cloudwatch:$(git log -1 --format=%h src/statsd-to-cloudwatch)"
...
deploy_images() {
    for f in statsd-to-cloudwatch colorapp feapp; do
    ...
    docker build -t ${STATSD_CW_IMAGE} ${DIR}/src/statsd-to-cloudwatch && docker push ${STATSD_CW_IMAGE}
...
deploy() {
    ...
    aws cloudformation deploy \
        ...
        --template-file "$DIR/deploy/app.yaml" \
        --capabilities CAPABILITY_IAM \
        --parameter-overrides \
        "ProjectName=${PROJECT_NAME}" \
        "EnvoyImage=${ENVOY_IMAGE}" \
        "StatsdCWImage=${STATSD_CW_IMAGE}" \
        "CloudWatchNamespace=${PROJECT_NAME}" \
